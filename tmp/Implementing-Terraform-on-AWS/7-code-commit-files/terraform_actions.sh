...
then
  echo "Running plan"
  terraform plan -out vpc.tfplan -no-color > tf_output.txt
...
then
  ...
  terraform apply vpc.tfplan -no-color > tf_output.txt
fi

if [ $TF_ACTION = "DESTROY" ] 
then
  echo "Running destroy"
  terraform destroy -auto-approve -no-color > tf_output.txt
fi